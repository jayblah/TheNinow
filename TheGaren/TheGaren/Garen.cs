...
namespace TheGaren
{
    class Garen
    {
        ...
        public void Load()
        {
            ...
            comboMenu.AddMItem("Q to Gapclose", true, (sender, args) => _comboProvider.GetSkill<GarenQ>().UseWhenOutOfRange = args.GetNewValue<bool>());

            miscMenu.AddMItem("Use W Out of Combo", true, (sender, args) => _comboProvider.GetSkill<GarenW>().UseAlways = args.GetNewValue<bool>());
            miscMenu.AddMItem("Min. % Incoming DPS for W", new Slider(2, 1, 15), (sender, args) => _comboProvider.GetSkill<GarenW>().MinDamagePercent = args.GetNewValue<Slider>().Value);
            ...
            drawingMenu.AddMItem("Draw Flash-Ult Indicator", new Circle(true, Color.Red), (sender, args) => _drawFlashUlt = args.GetNewValue<Circle>());
        ...
        private void Draw(EventArgs args)
        {
            ...
            if (_drawFlashUlt.Active && _r.IsReady())
            {
                foreach (var enemy in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(_flash != null && _flash.IsReady() ? 375 + 425 : 375) && _r.IsKillable(enemy)))
                {
                    ...
                    Drawing.DrawText(screenPos.X - 50, screenPos.Y - 50, _drawFlashUlt.Color, "Flash-Ult Possible!");
                }
            }
